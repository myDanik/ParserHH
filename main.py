...
def get_links_vacancy(text: str, education: Vacancy_Education, part_time: Vacancy_PartTime, experience: Vacancy_Experience, schedule: Vacancy_Schedule, count: int) -> List[str]:
    # text - Любой
    ...
    for page in range(total_pages):
        ...
        if data.status_code != 200:
            continue
        soup = BeautifulSoup(data.content, features="lxml")
        if soup.find(attrs={"class": "vacancy-serp-content"}):
...
# if __name__== "__main__":
#     for link in get_links_vacancy(text="python", schedule="fullday", education="higher", part_time="employment_part", experience=None, count=2):
#         print(get_vacancy(link))
